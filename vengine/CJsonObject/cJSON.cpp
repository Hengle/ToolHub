	...
	if (str) {
		if (item->sign == -1) {
			...
			if (item->valueint <= (uint64)UINT_MAX) {
				sprintf(str, "%u", (uint32_t)item->valueint);
			} else {
...
static char* print_value(cJSON* item, int32_t depth, int32_t fmt) {
	...
	switch ((item->type) & 255) {
		case cJSON_NULL:
			out = cJSON_strdup("null"_sv);
			break;
		case cJSON_False:
			out = cJSON_strdup("false"_sv);
			break;
		case cJSON_True:
			out = cJSON_strdup("true"_sv);
			break;
		case cJSON_Int:
			out = print_int(item);
			break;
...
cJSON* cJSON_CreateIntArray(int32_t* numbers, int32_t sign, int32_t count) {
	...
	for (i = 0; a && i < count; i++) {
		n = cJSON_CreateDouble((long double)((uint32_t)numbers[i]), sign);
		if (!i)
			a->child = n;
		else
			suffix_object(p, n);
