...
void FileUtility::GetFiles(vstd::string const& path, Runnable<void(vstd::string&&)> const& func) {
	uint64 hFile = 0;
...
void FileUtility::GetFiles(vstd::string const& path, Runnable<void(vstd::string&&)> const& func, Runnable<bool(vstd::string const&, bool)> const& ignoreFileFunc) {
	uint64 hFile = 0;
	struct _finddata_t fileinfo;
	if ((hFile = _findfirst((path + "/*"_sv).c_str(), &fileinfo)) != -1) {
		do {
