...
inline Math::Vector4 XM_CALLCONV clamp(
	...
	FXMVECTOR const& V = (FXMVECTOR const&)v;
	FXMVECTOR const& Min = (FXMVECTOR const&)min;
	FXMVECTOR const& Max = (FXMVECTOR const&)max;
#if defined(_XM_NO_INTRINSICS_)
...
inline Math::Vector4 XM_CALLCONV mul(const Math::Matrix4& m, const Math::Vector4& vec) noexcept {
	Math::Matrix4 const& mat = (Math::Matrix4 const&)m;
	return {
		dot(mat[0], vec),
		dot(mat[1], vec),
		dot(mat[2], vec),
...
inline Math::Vector3 XM_CALLCONV mul(const Math::Matrix3& m, const Math::Vector3& vec) noexcept {
	Math::Matrix4 const& mat = (Math::Matrix4 const&)m;
	return {
		CombineVector3(reinterpret_cast<Math::Vector3 const&>(mat[0]) * vec),
		CombineVector3(reinterpret_cast<Math::Vector3 const&>(mat[1]) * vec),
		CombineVector3(reinterpret_cast<Math::Vector3 const&>(mat[2]) * vec)};
}
...
inline bool XM_CALLCONV equal(const Math::Vector3& vec1, const Math::Vector3& vec2, float tolerance) {
	Math::Vector3 diff = abs(vec1 - vec2);
	return diff.x <= tolerance && diff.y <= tolerance && diff.z <= tolerance;
}
inline bool XM_CALLCONV equal(const Math::Vector4& vec1, const Math::Vector4& vec2, float tolerance) {
	Math::Vector4 diff = abs(vec1 - vec2);
	return diff.x <= tolerance && diff.y <= tolerance && diff.z <= tolerance && diff.w <= tolerance;
}
inline bool XM_CALLCONV equal(const Math::Vector2& vec1, const Math::Vector2& vec2, float tolerance) {
	Math::Vector2 diff = vec1 - vec2;
...
#ifdef _XM_NO_INTRINSICS_
	return Math::Vector3(XMVectorMax((FXMVECTOR const&)vec1, (FXMVECTOR const&)vec2));
#else
...
#ifdef _XM_NO_INTRINSICS_
	return Math::Vector4(XMVectorMax((FXMVECTOR const&)vec1, (FXMVECTOR const&)vec2));
#else
	return _mm_max_ps((XMVECTOR const&)vec1, (XMVECTOR const&)vec2);
#endif
}
...
#ifdef _XM_NO_INTRINSICS_
	return Math::Vector3(XMVectorMin((FXMVECTOR const&)vec1, (FXMVECTOR const&)vec2));
#else
...
#ifdef _XM_NO_INTRINSICS_
	return Math::Vector4(XMVectorMin((FXMVECTOR const&)vec1, (FXMVECTOR const&)vec2));
#else
	return _mm_min_ps((XMVECTOR const&)vec1, (XMVECTOR const&)vec2);
#endif
}

inline float XM_CALLCONV clamp(
	float a,
	float minV,
	float maxV) {
	a = Min(a, maxV);
