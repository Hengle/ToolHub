...
#include <Yaml/yaml-cpp/binary.h>
...
template<>
struct convert<vstd::string_view> {
	static Node encode(vstd::string_view sv) {
		return Node(sv);
	}
	static bool decode(const Node& node, vstd::string_view& rhs) {
		if (!node.IsScalar())
			return false;
		auto&& ss = node.Scalar();
		rhs = vstd::string_view(ss.data(), ss.size());
		return true;
	}
};
template<>
...
template<typename T>
typename std::enable_if<(std::is_same<T, unsigned char>::value || std::is_same<T, signed char>::value), bool>::type
