...
namespace MPipeline
{
    ...
    public unsafe struct NativeDictionary<K, V, F> : IEnumerable<Pair<K, V>> where K : unmanaged where V : unmanaged where F : unmanaged, IFunction<K, K, bool>
    {
        ...
        private static int GetPow2Sizde(int targetSize)
        {
            int tempSize = 1;
        ...
        private void Resize(int targetSize)
        {
            if (targetSize <= Capacity)
                return;
            targetSize = GetPow2Sizde(targetSize);
            ...
            PtrList* pooledList = MUnsafeUtility.Malloc<PtrList>(sizeof(PtrList) * Capacity, Allocator.Temp);
            int pooledCount = 0;
            ...
            for (int i = 0; i < data->keyValueList.length; ++i)
            {
                ...
                uint newHash = hash & ((uint)targetSize - 1);
                AddKey(newHashArray, newHash, ref *keyPtr, i, pooledList, ref pooledCount);
            }
            UnsafeUtility.Free(pooledList, Allocator.Temp);
        ...
        public NativeDictionary(int capacity, Allocator alloc, F equalFunc)
        {
            ...
            capacity = GetPow2Sizde(capacity);
        ...
        private void ResetValue(int index)
        {
            ...
            uint hash = (uint)targetKey->GetHashCode() & ((uint)Capacity - 1);
        ...
        {
            uint hash = (uint)key.GetHashCode() & ((uint)Capacity - 1);
            ref PtrList lst = ref data->hashArray[hash];
            for (int i = 0; i < lst.length; ++i)
            {
                K* keyPtr = (K*)lst[i];
