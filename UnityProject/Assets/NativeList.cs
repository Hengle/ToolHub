...
namespace MPipeline
{
    ...
    public unsafe struct NativeList<T> : IEnumerable<T> where T : unmanaged
    {
        ...
        public NativeList(int capacity, in Allocator alloc)
        {
            isCreated = true;
        ...
        public NativeList(in int count, in Allocator alloc, in T defaultValue)
        {
            isCreated = true;
            data = MUnsafeUtility.Malloc<NativeListData>(sizeof(NativeListData), alloc);
            data->count = count;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public NativeList(in int count, in int capacity, in Allocator alloc)
        {
            isCreated = true;
            data = MUnsafeUtility.Malloc<NativeListData>(sizeof(NativeListData), alloc);
            data->count = count;
        ...
        public void AddCapacityTo(in int capacity)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void RemoveLast(in int length)
        {
            data->count -= length;
        ...
        {
            NativeArray<T> arr = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<T>(unsafePtr, Length, Allocator.None);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void AddRange(in int length)
        {
            data->count += length;
            ResizeToCount();
        }
        ...
        public void AddRange(in T* array, in int length)
        {
            int last = data->count;
            data->count += length;
            ResizeToCount();
        ...
        public int ConcurrentAdd(in T value)
        {
        ...
        public int ConcurrentAdd(in T value, in object lockerObj)
        {
            int last = Interlocked.Increment(ref data->count);
            //Concurrent Resize
            if (last > data->capacity)
